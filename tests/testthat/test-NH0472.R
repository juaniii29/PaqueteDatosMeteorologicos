...
# Test para verificar que el dataset `NH0472` tiene las columnas correctas
test_that("NH0472 dataset tiene las columnas correctas", {
  # Cargar el dataset
  NH0472 <- get_data("NH0472")

  # Verificar el número de columnas y sus nombres
  expect_equal(ncol(NH0472), 11)
  ...
  expect_equal(colnames(NH0472), expected_columns)
})
...
test_that("get_data descarga y lee el archivo correctamente", {
  ...
  NH0910 <- get_data("NH0472")
  expect_true(nrow(NH0472) > 0)
  expect_equal(ncol(NH0472), 35)
})
...
test_that("get_data lanza un error con una URL incorrecta", {
  ...
  expect_error(
    {
      url <- incorrect_url
      destfile <- file.path(getwd(), "data_raw", "NH0472_incorrect.csv")
...
test_that("get_data lanza un error si el archivo descargado no es un CSV válido", {
  destfile <- file.path(getwd(), "data_raw", "NH0472_invalid.csv")
  ...
  expect_error(get_data("NH0472_invalid"), "No se pudo leer el archivo descargado. Verifique que el archivo sea un CSV válido.")
